"""Natural language to Python code generation using LlamaIndex and GPT 4o mini.

The agent receives the user question plus an explicit Postgres schema and
strict output protocol. It must return a single self contained Python
script that:

* Reads from Postgres via SQLAlchemy.
* Uses Polars, Pandas, Plotly, Prophet, or Statsmodels as appropriate.
* Writes either a CSV table or a Plotly figure JSON to stdout between
  marker tags so :mod:`output_parser` can recover them.

This module never executes the generated code itself. Execution is the job
of one of the four sandboxes in :mod:`sandboxes`.
"""
...
Table: model_repos
  ...
  tag             VARCHAR        - Hugging Face category, e.g. 'text-generation'
  author          VARCHAR
  likes           INTEGER        - proxy for stars
  downloads       INTEGER        - proxy for forks or usage
...
Table: discussions
  id                  INTEGER PRIMARY KEY
  repo_id             INTEGER  - foreign key to model_repos.id
...
Connection:
    ...
    engine = create_engine(DATABASE_URL)  # DATABASE_URL is already in scope
"""

CODE_GEN_SYSTEM_PROMPT = f"""You are a senior Python data engineer. Given a user's
natural language analytics question about Hugging Face model repositories, you
must generate a SINGLE self contained Python 3.11 script that answers it.
...
3. For TEXT or TABLE answers:
     ...
     - then print the resulting table as CSV. EACH MARKER MUST BE ON ITS OWN LINE:
           ...
           print('<<<TABLE_END>>>')
4. For CHART answers (line, pie, bar, stacked, forecast, ANY visualization):
     - Build a plotly.graph_objects.Figure or a plotly.express figure.
...
7. Polars API reminders (use these EXACT signatures, NOT pandas equivalents):
...
def generate_python_code(question: str) -> GeneratedCode:
    ...
    llm = OpenAI(
        model=cfg.openai_model, api_key=cfg.openai_api_key, temperature=0
    )
    messages = [
