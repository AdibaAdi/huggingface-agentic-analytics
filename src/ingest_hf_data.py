"""Ingestion script: pulls top N Hugging Face models per tag + their discussions
and stores them in Postgres tables. Run once before launching Streamlit.
"""
...
def ingest_data() -> dict:
        ...
        for tag in cfg.hf_tags:
            ...
            for model in models:
                ...
                repo = session.execute(
                    select(ModelRepo).where(ModelRepo.model_id == model.id)
                ).scalar_one_or_none()
                if repo is None:
                ...
                try:
                    ...
                    for d in discussions:
                        ...
                        if existing is None:
                            existing = Discussion(
                                repo_id=repo.id,
                                hf_discussion_num=int(getattr(d, "num", 0)),
                                title="",
                            )
                        ...
                        existing.status = (
                            "closed" if bool(getattr(d, "status", "") == "closed") else "open"
                        )
    ...
    summary["limitations"].append(
        "Hugging Face does not expose GitHub-style commits/PR timelines for all repos; "
        "downloads serves as a 'forks/usage' proxy and is_pull_request flags act as PR proxy."
    )
    return summary
